...
const Register: React.FC<RegisterProps> = ({ onRegister, onGoogleLogin, onSwitchToLogin, error }) => {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-6">
      <div className="w-full max-w-md space-y-10">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-light text-slate-900 tracking-tight">יצירת חשבון חדש</h1>
          <p className="text-slate-400 font-light">הצטרפו לאלפי בעלי עסקים שכבר בונים עם Eve.</p>
        </div>
        ...
        <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-[3rem] p-10 shadow-sm space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              <label className="text-[12px] font-medium text-slate-400 uppercase tracking-widest px-4">שם מלא</label>
              <input
                type="text"
                value={name}
        ...
        <p className="text-center text-slate-400 font-light">
          כבר יש לך חשבון?{' '}
          <button onClick={onSwitchToLogin} className="text-[#e33670] font-medium hover:underline">
            התחברות
          </button>
        </p>
      </div>
